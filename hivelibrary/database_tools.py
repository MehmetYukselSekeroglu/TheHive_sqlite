...
def connection_function(db_config_dict) -> tuple[sqlite3.Connection, sqlite3.Cursor]:

    cnn= sqlite3.connect(**db_config_dict)
    cursor = cnn.cursor()
    return cnn, cursor


def check_db_init_status(db, db_cursor:sqlite3.Cursor) -> bool:
    try:
        STATIC_SQL_COMMAND = "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name=?)"
        ...
        STATIC_SQL_COMMAND = f"SELECT EXISTS (SELECT 1 FROM {DB_SYSTEM_TABLE} WHERE unique_key=?)"
...
def check_exists_systemTable(db_curosr:sqlite3.Cursor, sql_key:str) -> list[bool, str]:
    try:
...
def insertData_systemTable(db:sqlite3.Connection, db_cursor:sqlite3.Cursor, sql_key:str, key_value:str) -> dict:
    try:
        ...
        STATIC_SQL_COMMAND = f"INSERT INTO {DB_SYSTEM_TABLE} (unique_key, value) VALUES (?, ?)"
